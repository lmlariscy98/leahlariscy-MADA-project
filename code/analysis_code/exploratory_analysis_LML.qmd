...
sum_daily_copies_county <- all_data_combine %>% 
  ...
  sum_daily_copies_county %>% ggplot(aes(log10(sum_total_copies))) +
  geom_histogram()
...
### Look at case 7 day moving average over time
...
all_data_combine %>% group_by(date) %>% 
  ...
  ggplot(aes(x = date, y = mean_7dma)) +
  geom_bar(stat = "identity", color = '#634490') +
...
all_data_combine %>% #filter(target == "N2") %>% 
  #drop_na() %>% #remove NAs from analysis
