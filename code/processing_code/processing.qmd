...
```{r}
n1_n2_plant <- left_join(wbe_year2_avg, plant_clean, by = c("date","facility")) %>% 
  mutate(total_copies = avg_copy_L * influent_flow_L)
...
`pos_rate_tr` will calculate the percento of positive technical replicates, averaged across biological replicates, within each sample's targets (N1 and N2)
