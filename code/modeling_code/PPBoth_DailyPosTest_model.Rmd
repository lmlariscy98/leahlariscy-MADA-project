...
```{r}
recipe_7dma <- recipe(pcr_pos ~ percent_pos, data = train_data)
```
...
huber <- aug_test %>% huber_loss(truth = pcr_pos, .pred)
...
aug_test %>% ggplot(aes(x=date)) +
  geom_point(aes(y=pcr_pos), color = "blue") +
...
```{r}
rmse <- aug_test2 %>% rmse(truth = pcr_pos, .pred)
rsq <- aug_test2 %>% rsq(truth = pcr_pos, .pred)
huber <- aug_test2 %>% huber_loss(truth = pcr_pos, .pred)
...
aug_test3 %>% select(pcr_pos, .pred)
```

## Evaluate model performance on testing dates

```{r}
rmse <- aug_test3 %>% rmse(truth = pcr_pos, .pred)
rsq <- aug_test3 %>% rsq(truth = pcr_pos, .pred)
huber <- aug_test3 %>% huber_loss(truth = pcr_pos, .pred)
...
aug_test3 %>% ggplot(aes(x=date)) +
  geom_line(aes(y=pcr_pos), color = "#621940", size = 2) +
  theme_light()
...
```{r, message=FALSE}
 aug_test3 %>% ggplot(aes(x=date)) +
   geom_line(aes(y=pcr_pos), color = "#1c246c", size = 2) +
   geom_line(aes(y=.pred), color = "#ec4998", size = 2) +
   theme_light() +
     ylab(label = "Predicted Daily Clinical Cases") +
  xlab(label = "Date") +
  theme_light()
  
ggsave(here("figures/modeling/predicted_DailyCase.png"))
```
